"""
illumorae_ascii_clean.py - Scan the ComfyUI_ILLUMORAE repository for disallowed
...
Adapted from tools/ref/DEV_ascii_clean.py for the ComfyUI_ILLUMORAE layout:
default targets are the ComfyUI_illumorae_* node packages plus tools, docs,
and tests; output is written under tools/ascii_clean_reports/.

Usage:
    python tools/illumorae_ascii_clean.py
    python tools/illumorae_ascii_clean.py --iterations
    python tools/illumorae_ascii_clean.py --fix --apply
    python tools/illumorae_ascii_clean.py --fix --fix-unknown --apply
    python tools/illumorae_ascii_clean.py --filter .md
    python tools/illumorae_ascii_clean.py --targets ComfyUI_illumorae_ImageInfillExemplarRegionFill

Output structure (default):
    tools/ascii_clean_reports/ascii_clean_<YYYYMMDD_HHMMSS>.txt

With --iterations:
    tools/ascii_clean_reports/iterations/<DATE>/ascii_clean_report.md
    tools/ascii_clean_reports/iterations/<DATE>/summary.json
...
# the repo root, so "ComfyUI_illumorae_*" matches every node package.
DEFAULT_TARGETS = ["ComfyUI_illumorae_*", "tools", "docs", "tests"]

def default_output_path() -> Path:
    """Return a datetime-prefixed output path under tools/ascii_clean_reports/."""
...
# Directories to skip entirely (only non-source cache/build/venv dirs;
...
SKIP_PATH_PATTERNS = {
    ...
    "tools/illumorae_ascii_clean.py",
    # Exclude the reference tool this was adapted from (same reason: it
    # contains literal DISALLOWED character tables that self-trigger)
    "tools/ref/DEV_ascii_clean.py",
    # Output folders produced by this tool itself
    "tools/ascii_clean_reports/*",
    ...
    "tools/ascii_clean_reports/iterations/*",
    # Generated linter / validation reports. These are produced by
    # illumorae_node_linter_runner.py and contain serialized diffs of
    # source code with json.dumps(ensure_ascii=True), so every non-ASCII
    # char in the audited source becomes a \uXXXX escape in the JSON.
    # They are already in .gitignore and are stale snapshots, not prose.
    "*/node_linter_report.json",
    "*/node_validation_report.json",
}
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out", type=Path, default=None,
                   help="Output path (default: tools/ascii_clean_reports/ascii_clean_<YYYYMMDD_HHMMSS>.txt)")
    p.add_argument("--targets", nargs="+", default=DEFAULT_TARGETS,
                   help=f"Subdirectories to scan relative to repo root. Glob patterns supported "
                        f"(e.g. ComfyUI_illumorae_*). Default: {DEFAULT_TARGETS}")
